...
class Track:
    def __init__(self, track_id, z):
        ...
        self.last_z = np.array([z[0], z[1]], dtype=np.float64)

        self.has_velocity_init = False
    ...
    def update(self, z, R, dt):
        if dt > 1e-3:
            dz = z - self.last_z
            speed = np.linalg.norm(dz) / dt

            if speed > 0.03:
                theta = math.atan2(dz[1], dz[0])

                # 不要讓速度突然爆掉，限制在合理行人速度
                speed = min(speed, 1.5)

                if not self.has_velocity_init:
                    self.X[2] = speed
                    self.X[3] = theta
                    self.has_velocity_init = True
                else:
                    # 平滑更新速度與方向
                    self.X[2] = 0.7 * self.X[2] + 0.3 * speed

                    old_theta = self.X[3]
                    angle_diff = math.atan2(
                        math.sin(theta - old_theta),
                        math.cos(theta - old_theta)
                    )
                    self.X[3] = old_theta + 0.3 * angle_diff
                    self.X[3] = math.atan2(math.sin(self.X[3]), math.cos(self.X[3]))
...
class EKFTracker:
    ...
    def update_tracks(self, measurements, dt):
        ...
        for i, j in pairs:
            self.tracks[i].update(measurements[j], self.R, dt)
    ...
    def get_front_person_avoid_goal(self):
        ...
        for track in self.active_tracks:
            candidate_global_positions = [
                ...
                (*self.predict_track_position(track, 0.5), 0.5),
                (*self.predict_track_position(track, 1.0), 1.0),
                (*self.predict_track_position(track, 1.5), 1.5),
                (*self.predict_track_position(track, 2.0), 2.0),
                (*self.predict_track_position(track, 2.5), 2.5)
            ]
