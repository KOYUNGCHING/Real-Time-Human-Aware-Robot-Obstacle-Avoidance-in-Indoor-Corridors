...
9. Constant-speed motion:
   The robot does NOT stop or slow down during normal avoidance.
   DWA only selects angular velocity w. Linear velocity v is fixed.
10. Wall emergency protection:
   If the robot is about to hit a wall / box / cabinet, it stops forward motion
   and rotates in place to find a safe direction.
"""
...
class RealTimeLegDetector:
    def __init__(self):
        ...
        # 只有快撞到時，才暫時原地旋轉。
        # =====================================================
        self.wall_emergency_enabled = True

        # 前方低於這個距離就進入緊急模式
        self.wall_front_stop_distance = 0.45

        # 正前方檢查區域
        self.wall_front_check_x = 0.65
        self.wall_front_check_y = 0.35

        # 左右太近也進入緊急模式
        self.wall_side_stop_distance = 0.23

        # 左右側檢查區域
        self.wall_side_check_x_min = -0.05
        self.wall_side_check_x_max = 0.60
        self.wall_side_check_y = 0.45

        # 原地旋轉速度
        self.wall_escape_turn_speed = 0.65

        # 前方恢復安全距離，要比 stop distance 大一點，避免一直進出緊急模式
        self.wall_front_release_distance = 0.60

        # 緊急模式狀態
        self.wall_emergency_active = False
        self.last_wall_escape_w = self.wall_escape_turn_speed
        ...
        rospy.loginfo("Real-time leg tracker with constant-speed EKF + DWA + wall emergency protection is ready.")
    ...
    def get_wall_emergency_turn(self):
        """
        牆壁 / 箱子 / 櫃子緊急保護。
        ...
        特點：
        - 不調高 DWA wall_weight。
        - 只有前方真的太近，才 stop + rotate。
        - 轉向會選比較空的方向。
        """
        if not self.wall_emergency_enabled:
            return None

        if self.latest_scan_pts is None or len(self.latest_scan_pts) == 0:
            return None

        pts = self.latest_scan_pts
        ...
        front_mask = (
            (x > 0.05)
            & (x < self.wall_front_check_x)
            & (np.abs(y) < self.wall_front_check_y)
        )

        # 左側近距離區域
        left_mask = (
            (x > self.wall_side_check_x_min)
            & (x < self.wall_side_check_x_max)
            & (y > 0.0)
            & (y < self.wall_side_check_y)
        )
        ...
        if np.any(front_mask):
            min_front_dist = float(np.min(np.linalg.norm(pts[front_mask], axis=1)))

        if np.any(left_mask):
            min_left_dist = float(np.min(np.linalg.norm(pts[left_mask], axis=1)))

        if np.any(right_mask):
            min_right_dist = float(np.min(np.linalg.norm(pts[right_mask], axis=1)))

        front_danger = min_front_dist < self.wall_front_stop_distance
        left_danger = min_left_dist < self.wall_side_stop_distance
        right_danger = min_right_dist < self.wall_side_stop_distance

        # 如果已經在 emergency active，使用比較寬鬆的 release 條件
        if self.wall_emergency_active:
            front_safe = min_front_dist > self.wall_front_release_distance
            side_safe = (
                min_left_dist > self.wall_side_stop_distance
                and min_right_dist > self.wall_side_stop_distance
            )

            if front_safe and side_safe:
                self.wall_emergency_active = False
                rospy.loginfo_throttle(
                    0.5,
                    "Wall emergency released: front={:.2f}, left={:.2f}, right={:.2f}".format(
                        min_front_dist,
                        min_left_dist,
                        min_right_dist
                    )
            ...
            if not front_danger and not left_danger and not right_danger:
                return None

            self.wall_emergency_active = True

        # 計算左右哪一邊比較空
        if np.any(left_mask):
            left_clearance = float(np.mean(np.linalg.norm(pts[left_mask], axis=1)))
        else:
            left_clearance = 5.0

        if np.any(right_mask):
            right_clearance = float(np.mean(np.linalg.norm(pts[right_mask], axis=1)))
        else:
            right_clearance = 5.0

        # 左邊太近，往右轉
        if left_danger and not right_danger:
            escape_w = -self.wall_escape_turn_speed
        ...
        # 前方太近，選比較空的方向
            ...
            if left_clearance > right_clearance + 0.05:
                ...
                # 左右差不多就沿用上一個方向，避免抖動
                if abs(self.last_wall_escape_w) > 0.01:
                    escape_w = self.last_wall_escape_w
                else:
                    escape_w = self.wall_escape_turn_speed

        self.last_wall_escape_w = escape_w

        rospy.loginfo_throttle(
            0.5,
            "Wall emergency active: front={:.2f}, left={:.2f}, right={:.2f}, left_clear={:.2f}, right_clear={:.2f}, escape_w={:.2f}".format(
                min_front_dist,
                min_left_dist,
                min_right_dist,
                left_clearance,
    ...
    def move_robot(self, event):
        ...
        # 這裡不調高 wall_weight。
        # =====================================================
        wall_escape_w = self.get_wall_emergency_turn()

        if wall_escape_w is not None:
            self.current_v = 0.0
            self.current_w = wall_escape_w

            move_cmd = Twist()
            move_cmd.linear.x = 0.0
            move_cmd.angular.z = wall_escape_w
    ...
    def scan_callback(self, msg):
        ...
        if ranges.size == 0:
            self.publish_delete_all(self.odom_frame_id)
            self.latest_scan_pts = np.empty((0, 2), dtype=np.float64)
            return
    ...
    def publish_track_markers(self, tracks, frame_id, dt):
        ...
        if self.current_pose is not None:
            ...
            m_robot.pose.position.x = self.current_pose[0]
            m_robot.pose.position.y = self.current_pose[1]
        ...
        if self.global_goal is not None:
            ...
            m_goal.pose.position.x = self.global_goal[0]
            m_goal.pose.position.y = self.global_goal[1]
        ...
        # 顯示回歸路線前視點
        if self.current_pose is not None and self.global_goal is not None:
            return_goal = self.get_return_goal_on_start_line()
            ...
            if return_goal is not None:
                ...
                m_return.pose.position.x = return_goal[0]
                m_return.pose.position.y = return_goal[1]
        ...
        # 顯示目前避障 local goal 轉 global 的點
        if self.current_pose is not None and self.latest_avoid_goal is not None:
            cx, cy, cyaw = self.current_pose

            agx, agy = self.local_to_global(
                self.latest_avoid_goal[0],
                self.latest_avoid_goal[1],
                cx,
                cy,
                cyaw
            )
            ...
            m_avoid.pose.position.x = agx
            m_avoid.pose.position.y = agy
        ...
        for t in tracks:
            ...
            m_person.pose.position.x = t.X[0]
            m_person.pose.position.y = t.X[1]
            ...
            m_text.pose.position.x = t.X[0]
            m_text.pose.position.y = t.X[1]
