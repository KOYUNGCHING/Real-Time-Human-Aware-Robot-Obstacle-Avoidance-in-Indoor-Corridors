...
class DWAPlanner:
    ...
    def plan(self, state, goal, static_scan, active_tracks, robot_pose_global):
        # 10. 牆壁/障礙物緊急保護：如果距離太近，強制停止前進並原地旋轉
        if len(static_scan) > 0:
            dists_to_wall = np.linalg.norm(static_scan, axis=1)
            if np.min(dists_to_wall) < (self.robot_radius + 0.05):
                rospy.logwarn_throttle(1.0, "!!! 緊急防撞觸發：前方過於接近牆壁，強制原地旋轉 !!!")
                return 0.0, 0.5
        ...
        # 10. 如果所有前進路線皆發生碰撞 (min_cost 為 inf)，原地旋轉尋找安全方向
        if min_cost == float('inf'):
            rospy.logwarn_throttle(1.0, "!!! DWA 找不到安全前進路徑，原地旋轉尋找出路 !!!")
            return 0.0, 0.5
...
class RealTimeLegDetector:
    ...
    def move_robot(self, event):
        ...
        # 10. 如果觸發了緊急保護（raw_v == 0.0 且正在旋轉），就讓速度等於 0
        if raw_v == 0.0 and abs(raw_w) > 0.01:
            best_v = 0.0
        else:
    ...
    def publish_track_markers(self, tracks, frame_id, dt, current_pose):
        ...
        # [修改] 顯示機器人自身位置 (因為是局部座標，機器人固定在相對原點 (0,0))
        m_robot = Marker()
