...
8. Early front-person avoidance:
   As soon as a tracked person is detected in the front corridor region,
   generate a side local goal to avoid earlier.
"""
...
class DWAPlanner:
    def __init__(self):
        ...
        # 避障時不要原地轉。
        # 只要 local goal 有明顯側向偏移，就要求 DWA 選擇邊走邊轉。
        self.min_forward_speed_when_avoiding = 0.04
        ...
        # 因為現在前方有人會直接改 local_goal，所以 heading 可以高一點
        ...
        # 讓轉向更順：不要偏好急轉，也不要偏好一邊停一邊轉
        # 轉速懲罰降低：避免轉得太慢
        # 仍保留 spin_in_place_weight，避免原地轉
        self.yaw_rate_weight = 0.10
    ...
    def plan(self, state, goal, static_scan, active_tracks, robot_pose_global):
        ...
        # 如果 goal 的 y 很大，代表目前正在做側向避障。
        # 這時禁止 DWA 選 v 太小的原地旋轉軌跡，讓機器人一邊走一邊轉。
    ...
    def calc_trajectory_cost(
        ...
        velocity_cost = (self.max_speed - v_cmd) / (self.max_speed - self.min_speed)

        if v_cmd < 0:
            velocity_cost += 2.0

        # 轉向平滑成本：w 越大成本越高，避免突然大角速度旋轉
        yaw_rate_cost = abs(w_cmd) / max(self.max_yaw_rate, 1e-6)

        # 原地轉 / 幾乎原地轉成本：避免出現「先轉完，再開始走」
...
class RealTimeLegDetector:
    def __init__(self):
        ...
        # 正前方提早閃避設定
        # 改法：不再是一看到人就閃。
        # 只有當 EKF 預測到人會出現在正前方 2m 內，才開始側向閃避。
        # =====================================================
        self.front_avoid_distance = 2.0   # 正前方 2m 內有預測到人才開始閃
        self.front_avoid_width = 0.75     # 只看正前方走廊寬度，避免旁邊的人誤觸發
        self.avoid_side_offset = 0.85     # 閃避時 local goal 往左右偏移多少
        self.avoid_forward_goal = 1.2     # 閃避時 local goal 放在前方多少距離
        self.center_dead_zone = 0.10      # 人幾乎在正中間時，固定往右閃

        self.latest_avoid_goal = None

        # 命令平滑參數：避免 v / w 一次跳太大
        # alpha 越小越平滑，但反應越慢；0.35 是比較穩的折衷
        self.cmd_smooth_alpha = 0.45

        # 只要正在轉，而且速度太小，就補一點前進速度，避免原地轉
        self.min_v_while_turning = 0.06

        # 閃避時不要減速：只要正在追側向避障點，就至少維持這個前進速度
        self.avoid_min_forward_speed = 0.08
        ...
        rospy.loginfo("Real-time leg tracker with EKF + DWA + front early avoidance is ready.")
    ...
    def predict_track_position(self, track, dt):
        """
        用 EKF state 預測某個 track 在 dt 秒後的位置。
        這裡只拿來做前方避障判斷，不會改變 track 本身。
        """
        x = track.X[0]
        y = track.X[1]
        v = track.X[2]
    ...
    def get_front_person_avoid_goal(self):
        """
        只有當 EKF 預測到人會出現在正前方 2m 內，才開始閃避。

        不再使用「一看到人就閃」的邏輯，避免太早被遠方或旁邊的人影響。

        判斷方式：
        - 檢查行人目前位置、1 秒後、2 秒後預測位置
        - 只要任一預測點在機器人正前方區域內，就觸發側向避障 local goal
        """
        self.latest_avoid_goal = None
        ...
        if self.start_pose is None:
            ...
            rospy.loginfo(
                "=== 終點設定完成: 起點前方 {:.1f} m, Goal X={:.2f}, Goal Y={:.2f} ===".format(
                    self.target_distance, goal_x, goal_y
                )
            )
    ...
    def move_robot(self, event):
        ...
        # =====================================================
        # 核心：
        # 如果 EKF 預測到人會進入正前方 2m 內，就切側向避障 local goal。
        # DWA 會被限制避障時不能原地轉，而是邊走邊轉。
        # 沒有人時，追回原始直線上的 return goal。
        # =====================================================
        ...
        state = [0.0, 0.0, 0.0, self.current_v, self.current_w]
        ...
        # =====================================================
        # 速度命令平滑：
        # 不要讓 v / w 每次直接跳到 DWA 給的最佳值。
        # 這樣機器人會比較像「一邊走一邊慢慢轉」，不會頓一下再轉。
        # =====================================================
        alpha = self.cmd_smooth_alpha
        best_v = (1.0 - alpha) * self.current_v + alpha * raw_v
        best_w = (1.0 - alpha) * self.current_w + alpha * raw_w

        # 如果正在轉，但是 v 太小，補一點前進速度，避免看起來像原地轉。
        # 只有在 local goal 在前方時才補，避免倒車或卡牆時硬衝。
        if abs(best_w) > 0.20 and best_v < self.min_v_while_turning and local_goal[0] > 0.2:
            best_v = self.min_v_while_turning

        # 閃人的時候不要減速：
        # local goal 有明顯側向偏移時，代表正在閃避，至少維持 avoid_min_forward_speed。
        is_front_avoiding = abs(local_goal[1]) > 0.30 and local_goal[0] > 0.2
        if is_front_avoiding and best_v < self.avoid_min_forward_speed:
            best_v = self.avoid_min_forward_speed

        # 限制輸出範圍，保險用
        best_v = max(self.dwa.min_speed, min(self.dwa.max_speed, best_v))
    ...
    def publish_track_markers(self, tracks, frame_id, dt):
        ...
        # 顯示目前避障 local goal 轉成 global 的位置
...
class RealTimeLegDetector:
    def __init__(self):
        ...
        # 只要正在轉，而且速度太小，就補一點前進速度，避免原地轉
        self.min_v_while_turning = 0.1

        # 閃避時不要減速：只要正在追側向避障點，就至少維持這個前進速度
        self.avoid_min_forward_speed = 0.13
