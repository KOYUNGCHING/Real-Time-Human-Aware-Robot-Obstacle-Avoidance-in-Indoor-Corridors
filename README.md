...
The Python ROS nodes mainly use:

```bash
pip install numpy
```

ROS dependencies may include:
...
Start ROS and make sure the robot or simulator is publishing LiDAR and odometry topics.

Typical subscribed topics:
...
```bash
python3 "adaboost_leg/最終模型和時時偵測/leg_detector.py"
```

### Run EKF + Hungarian Tracking

```bash
python3 "EKF+Hungarian/code/ekf_line.py"
```

or:

```bash
python3 "EKF+Hungarian/code/KO.py"
```

### Run DWA-VO Planner

```bash
python3 "DWA+VO/dwa_vo_success.py"
```

For RViz visualization:

```bash
python3 "DWA+VO/dwa_vo_success_rviz.py"
```

For the version with additional turning behavior when no safe path is available:

```bash
python3 "DWA+VO/dwa_vo_沒路時會轉頭.py"
```
